...
const Products = () => {
  ...
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [currentProduct, setCurrentProduct] = useState(null);
  const [formData, setFormData] = useState({ name: '', sku: '', category: '', price: '', cost: '', stock: '' });
  ...
  const handleEdit = (p) => {
    setCurrentProduct(p);
    setFormData({ name: p.name, sku: p.sku, category: p.category, price: p.price, cost: p.cost, stock: p.stock });
    setIsModalOpen(true);
  };

  const handleAdd = () => {
    setCurrentProduct(null);
    setFormData({ name: '', sku: '', category: '', price: '', cost: '', stock: '' });
    setIsModalOpen(true);
  };
  ...
  return (
    <div className="bg-gray-100 dark:bg-gray-900">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">{t('products')}</h1>
        {user?.role === 'admin' && (
          <button onClick={handleAdd} className="flex items-center px-4 py-2 rounded-md bg-indigo-600 dark:bg-indigo-700 text-white">
            <Plus size={20} className="mr-2" /> {t('add_product')}
          </button>
        )}
      </div>

      <div className="rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-hidden">
        <div className="p-4 border-b border-gray-200 dark:border-gray-700">
          <div className="relative">
            ...
            <input
              ...
              onChange={(e) => setSearchTerm(e.target.value)}
              placeholder={t('search')}
              className="w-full pl-10 pr-3 py-2 rounded-md bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-600"
            />
          </div>
        </div>
            ...
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {filteredProducts.map(p => (
                <tr key={p._id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">Rs {p.price}</td>
                  <td className="px-6 py-4 text-sm">
                    <span className={`px-2 text-xs font-semibold rounded-full ${p.stock < 10
                  ...
                  <td className="px-6 py-4 text-right">
                    {user?.role === 'admin' && (
                      <>
                        <button onClick={() => handleEdit(p)} className="text-indigo-600 dark:text-indigo-400 mr-4"><Edit size={18} /></button>
                        <button onClick={() => handleDelete(p._id)} className="text-red-600 dark:text-red-400"><Trash2 size={18} /></button>
                      </>
                    )}
                  </td>
                </tr>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={currentProduct ? 'Edit Product' : 'Add Product'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          {['name','sku','category','price','cost','stock'].map((field) => (
            <input
              key={field}
              placeholder={field}
              value={formData[field]}
              onChange={(e) => setFormData({ ...formData, [field]: e.target.value })}
              className="w-full p-2 rounded border bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 border-gray-300 dark:border-gray-600"
            />
          ))}
          <div className="flex justify-end gap-3">
            <button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 rounded border dark:border-gray-600">Cancel</button>
            <button type="submit" className="px-4 py-2 rounded bg-indigo-600 dark:bg-indigo-700 text-white">Save</button>
          </div>
        </form>
      </Modal>
    </div>
